...
function checkFill(id) {{
    ...
    const correct = answer.textContent.replace('💡 ', '').trim().toLowerCase();
    ...
    if (userAnswer === correct || correct.includes(userAnswer) || userAnswer.includes(correct) || correct.split(/[;；,，]/).some(p => p.trim() === userAnswer)) {{
